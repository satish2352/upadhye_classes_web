   ...
   public function __construct()
    ...
    public function store(Request $request) {
        $rules = [
            ...
            'image' => 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.OUR_RESULT_IMAGE_MAX_SIZE").'|dimensions:min_width=1500,min_height=500,max_width=2000,max_height=1000|min:'.Config::get("AllFileValidation.OUR_RESULT_IMAGE_MIN_SIZE").'',
    ...
    public function update(Request $request)
{
    $rules = [
     ...
     if($request->has('image')) {
        $rules['image'] = 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.OUR_RESULT_IMAGE_MAX_SIZE").'|dimensions:min_width=1500,min_height=500,max_width=2000,max_height=1000|min:'.Config::get("AllFileValidation.OUR_RESULT_IMAGE_MIN_SIZE");
    }
    if($request->has('marathi_image')) {
        $rules['marathi_image'] = 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.OUR_RESULT_IMAGE_MAX_SIZE").'|dimensions:min_width=1500,min_height=500,max_width=2000,max_height=1000|min:'.Config::get("AllFileValidation.OUR_RESULT_IMAGE_MIN_SIZE");
    }
    $messages = [   
        // 'category_id.required' => 'Please select category.',
        'image.required' => 'The image is required.',
